    ...
    Column {
        Column(modifier=Modifier.weight(1f)) {
            SearchBar(
                ...
                onSearch = { viewModel.searchPost(searchTerm) })
            PostList(
                isContentLoading = false,
                isPostLoading = viewModel.searchProgress.value,
                posts = viewModel.searchedPosts.value,
                modifier = Modifier
                    .padding(8.dp)
                    .fillMaxWidth()
                    .weight(1f)
...
fun SearchBar(searchTerm: String, onSearchChange: (String) -> Unit, onSearch: () -> Unit) {
    ...
    TextField(
        ...
        maxLines = 1,
        singleLine = true,
        colors = TextFieldDefaults.textFieldColors(containerColor = Color.Transparent, textColor = Color.Black,
